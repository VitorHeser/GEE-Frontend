...
export class GraficosanaliticosComponent implements OnInit {
  Empresas: any[] = []
  @Input() EmpresaSelected
  indicador: any[]=[]
  DadosAux: any[]=[]

  @Input() date2= new Date()
  @Input() date1 = new Date(new Date().setMonth(new Date().getMonth() - 11))

  SelecaoAguaEsg
  @Input() AguaEsgoSelect=1

  SelecaoIndicadores
  @Input() IndicadoresSelect: any= 'kwhm3';

  @Input() carregardevez: boolean= false;
    ...
    this.servic.importarUnidades().subscribe(
      result=>{
        for(var i =0;i<result.length;i++){
          ...
          this.EmpresaSelected =this.EmpresaSelected ==undefined ?  this.Empresas[0] : this.EmpresaSelected
        }
        this.carregarLabels();
        if(this.carregardevez){
          this.criarArrayDoGrafico()
        }
        this.carregado=true
      }
    )
  ...
  procurarCor(valor, id):any{
    ...
    var paleta=[
      ...
      {cor:           '#09ff006c', value: 'consumoEnergia', label: 'Consumo kWh'},
