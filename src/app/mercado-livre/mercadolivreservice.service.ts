...
export class MercadolivreserviceService {
  ...
  httpOptions = {headers: new HttpHeaders().set('Authorization',sessionStorage.getItem('token'))}
   ...
   UCS(): Observable<any[]>{
      return  this.http.get(`${API_CONFIG}/MercLivreUcs`,this.httpOptions) 
      .pipe(map((res : any[]) => res, catchError(ErrorHandler.handleError)))
   }
   UCSUpdateNew(unid: any): Observable<any[]>{
      return  this.http.put(`${API_CONFIG}/MercLivreUcs/${unid.id}`,unid,this.httpOptions) 
      .pipe(map((res : any[]) => res, catchError(ErrorHandler.handleError)))
   }
   UCSDelete(unid: any): Observable<any[]>{
      return  this.http.delete(`${API_CONFIG}/MercLivreUcs/${unid.id}`,this.httpOptions) 
      .pipe(map((res : any[]) => res, catchError(ErrorHandler.handleError)))
   }
